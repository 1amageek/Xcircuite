...
ElectricalSignoffEngine is registered as the electrical analysis and release-profile package. `ElectricalStandardLayoutImportFlowStageExecutor` converts verified DEF/GDSII/OASIS plus LEF technology inputs into a canonical physical snapshot and blocks missing connectivity; LEF inputs must carry an explicit digest-bound layer-map artifact because LEF does not define GDS layer numbers. The stage also persists an `ElectricalSignoffInputArtifactManifest` that records the exact source digests consumed by the run. `ElectricalSignoffFlowStageExecutor` uses `LocalElectricalArtifactStore`, persists the canonical run result and per-axis artifacts, and writes a Foundation `ElectricalSignoffFoundationEvidence` artifact with captured input/output lineage; `ElectricalSignoffQualificationFlowStageExecutor` persists corpus, retained qualification, immutable independent-oracle artifacts and its input manifest. Qualification can consume either a digest-bearing observation artifact or an `ElectricalSignoffOracleProcessConfiguration`; the latter runs an absolute executable with `{{specPath}}`, `{{outputPath}}`, `{{projectRoot}}` and `{{runID}}` substitutions, enforces a timeout/cancellation boundary, validates the produced observation set, and retains stdout, stderr and an `ElectricalSignoffOracleProcessExecution` record under the run artifact tree. `ElectricalSignoffProcessQualificationFlowStageExecutor` consumes a digest-bound `ElectricalSignoffProcessQualificationRequest`, requires complete PDK scope, exact case/report identity, independent oracle agreement, hashed corpus/oracle/health/approval/evidence artifacts and a bounded window, then persists structured checks and `ToolProcessQualificationEvidence`. `ElectricalSignoffRepairRevisionFlowStageExecutor` applies one provenance-checked repair candidate to a new physical-design revision; `ElectricalSignoffReleaseGateFlowStageExecutor` accepts only digest-bound artifact references or verified stage-artifact selectors, validates the retained qualification spec, consumes explicit run-result, qualification-report and policy references, verifies exact qualification case/corner provenance and persists the all-axis/all-corner release decision. Process-qualified policies additionally require a separate fresh, PDK-scoped `ToolProcessQualificationEvidence` artifact. All six electrical flow entries are constructible from typed `XcircuiteFlowStageExecutorSpec` runtime entries, so Agent and CLI flows do not need UI-only wiring. The release gate does not silently mutate design state or claim process qualification without external evidence.
