...
extension XcircuiteFlowCLICommand {
    ...
    public static var helpText: String {
        ...
        Usage:
          ...
          xcircuite-flow review-run --project-root <path> --run-id <id> [--pretty]
          xcircuite-flow approve-gate --project-root <path> --run-id <id> --stage-id <id> --verdict <approved|rejected> --reviewer <id> [--reviewer-kind <human|agent|cli|system>] [--note <text>] [--pretty]
          xcircuite-flow request-cancel --project-root <path> --run-id <id> --requested-by <id> --reason <text> [--pretty]
          xcircuite-flow progress-run --project-root <path> --run-id <id> [--since-sequence <n>] [--wait] [--timeout-ms <n>] [--poll-interval-ms <n>] [--pretty]
          ...
          xcircuite-flow scaffold-run --project-root <path> --run-id <id> --out-run-spec <run.json> --out-runtime-config <runtime.json> [--stage <kind>[,<kind>...]] [--pretty]
    ...
    public static var writeOpAmpEvaluationProfileHelpText: String {
        ...
        Writes an FlowEvaluationProfile for op-amp design evaluation channels and required artifact roles. The profile is evaluation material, not a fixed design loop.
        """
    }
    ...
    public static var scaffoldRunHelpText: String {
        ...
        to verified levels, then run validate with the same --project-root.
        """
    }
    ...
    public static var approveCandidatePlanRiskHelpText: String {
        ...
        Writes .xcircuite/runs/<run-id>/approvals/<approval-id>.json using the shared FlowApprovalRecord schema. The next verify-candidate-plan or execute-candidate-plan call reads the approval and updates riskReviews or pre-execution mutation policy.
        """
    }
