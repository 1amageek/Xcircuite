...
struct XcircuiteCandidatePlanGeneratorTests {
    @Test func generateCandidatePlanCLIReadsPlanningProblemFromRunManifest() async throws {
        ...
        #expect(result.candidatePlanArtifact.path == ".xcircuite/runs/run-1/planning/candidate-plan.json")
        #expect(result.candidatePlanArtifact.sha256?.isEmpty == false)
        #expect(result.candidatePlanArtifact.byteCount != nil)
        ...
        #expect(translationAuditArtifact.path == ".xcircuite/runs/run-1/planning/problem-translation-audit.json")
        ...
        #expect(actionDomainArtifact.path == ".xcircuite/runs/run-1/planning/action-domain-snapshot.json")
        #expect(actionDomainArtifact.sha256?.isEmpty == false)
        #expect(actionDomainArtifact.byteCount != nil)
        let traceArtifact = try #require(result.symbolicPlannerTraceArtifact)
        #expect(traceArtifact.artifactID == XcircuitePlanningArtifactStore.symbolicPlannerTraceArtifactID)
        #expect(traceArtifact.path == ".xcircuite/runs/run-1/planning/symbolic-planner-trace.json")
        #expect(traceArtifact.sha256?.isEmpty == false)
        #expect(traceArtifact.byteCount != nil)
    ...
    @Test func generateCandidatePlanRefreshesTamperedActionDomainSnapshotBeforeUse() throws {
        ...
        #expect(result.executionReadiness == "ready")
        let actionDomainArtifact = try #require(result.actionDomainSnapshotArtifact)
        let refreshedIntegrity = verifier.verify(actionDomainArtifact, projectRoot: root)
        #expect(refreshedIntegrity.status == .verified)
        #expect(actionDomainArtifact.sha256 != staleReference.sha256)
        ...
        #expect(manifestActionDomain.sha256 == actionDomainArtifact.sha256)
        #expect(manifestActionDomain.byteCount == actionDomainArtifact.byteCount)
    }
